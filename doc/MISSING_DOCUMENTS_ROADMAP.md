...
・基本経済・リソース品目 : ★★☆☆☆ (産業の抽象論はあるが、一次/二次/消費材の具体品目定義が未作成)
...
今後、段階的に策定・保存していくべきドキュメントを以下の5領域・計6件として定義します。

### ① 経済・資源品目・物流循環領域（最優先・実装必須）

- **ドキュメント候補名**: [`./discussion/industry_logistics/concept_resource_economy_and_logistics.md`](./discussion/industry_logistics/concept_resource_economy_and_logistics.md)（資源カテゴリ・通貨・物流循環設計書）
- **優先度**: **高（最優先）**
- **目的・策定すべき内容**:
  - **一次資源（採掘・採取）**: エネルギー（重水素/地熱）、鉱物（特殊鋼/レアアース）、バイオ（現地食糧/生体組織）。
  - **二次資源（加工・部品）**: 基礎工業部品（整備・補充の共通通貨）、電子・論理コア、特殊装甲板。
  - **兵站・軍事消費**: 弾薬、推進剤、交換用モジュール部品（遠隔地での戦闘団維持コスト）。
  - **通貨・クレジット流通**: 民生市場と国家予算の交換レート、インフレと物資調達。
