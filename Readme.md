# ALIEN COM GAME

## 概要

ゲーム特許は無認識なので自分で対応して。

## ゲーム内容

4Xストラテジー、Civ:BEリスペクト。
架空の入植惑星を部隊にした架空の人類の話。

## ドキュメント

プロジェクトの設計書・アイデア検討資料の構成や索引は以下にまとめています。

- [ドキュメント構成・目録 (doc/INDEX.md)](doc/INDEX.md)
- [ゲーム基本仕様設計書 (doc/GAME_DESIGN.md)](doc/GAME_DESIGN.md)

## 実装済み内容

- UI
- マップ

## 今後の機能

- 勢力システム
- ユニットシステム
- イベントシステム
- その他いい感じのシステム

## 導入方法

日本語・絵文字・アイコンフォントに UDEV Gothic (Nerd Fonts) を用いる。`assets/fonts/` に `UDEVGothicNF-Regular.ttf` および `UDEVGothicNF-Bold.ttf` を配置する。

## 利用時は改変を推奨

ゲーム特許は無認識なので自分で対応して。
このゲームは私専用なので、このシステムは改変して自分の好み・標準に合わせることを推奨する。
ゲーム名称を変更して好きなシステムに改変するとよろしい。

## 推奨環境

- CPU: intel core i3相当以上(2020年以降)
- メモリ: DDR4・16GB以上(2020年以降)
- GPU: VRAM8GB以上(2020年以降)
- SSD: 空き容量100GB以上(2020年以降)

## コマンド集

cargo clippy --fix  --all --allow-dirty

cargo coupling --ai

cargo coupling --japanese --web

## License

アセットに使用している外部素材のフォントを除いたファイルは、MITまたはApache2.0ライセンスとする。
